...
const PensumPage = () => {
  ...
  const { subjects, isLoading: loadingSubjects } = useSubject(
    selectedPensum?.id || 0
  );
  ...
  const enrichedSubjects = useMemo(() => {
    return subjects.map((subject) => {
      ...
      const academicProgram = academicPrograms.find(
        (program) => program.id === pensum?.academicProgramId
      );
      ...
      return {
        ...
        creditRequirementsV: subject.creditRequirements
          ? subject.creditRequirements
          : "-",
        version: pensum ? pensum.version : "Pensum desconocido",
        modality: academicProgram
          ? academicProgram.modalityAcademic
  ...
  return (
    <>
      <div className="py-2">
        <h1 className="text-h-2 text-primary-7740 mb-6">Pensum Académico</h1>
      </div>
      <CustomDropdown
        pensums={pensums}
