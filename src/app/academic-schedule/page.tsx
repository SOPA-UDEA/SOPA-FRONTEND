...
const Page = () => {
  const [academicSchedule, setAcademicSchedule] =
    useState<AcademicScheduleResponse | null>(null);
  ...
  // ✅ Texto correcto de los pensums seleccionados (usando id y versión)
  console.log(pensums, selectedPensumsIds)
  const selectedPensumsNames = pensums
  .filter((p) => selectedPensumsIds.includes(p.id))
  .map((p) => {
    // Definir modalidad según academicProgramId
    let modalidad = "";
    if (p.academicProgramId === 1) {
      modalidad = "presencial";
    } else if (p.academicProgramId === 2) {
      modalidad = "virtual";
    }
  ...
  const enrichedGroups = groups.map((group) => {
    ...
    const classrooms = Object.keys(groupedClassroom);
    const schedules = Object.values(groupedClassroom).map((list) => list.join(" "));
    ...
    return {
      ...
      subjectModality: group.subject.pensum.academic_program?.modalityAcademic ?? "N/A",
  ...
  return (
    <>
      ...
      <div>
        <h1 className="text-h-2 text-primary-7740 mb-4">
          Programación Académica
        </h1>

        {/* 🔹 Texto dinámico del pensum y semestre */}
        {academicSchedule && selectedPensumsIds.length > 0 && (
          <p className="text-gray-700 mb-6">
            <b>Programación académica:</b> {selectedPensumsNames} — Semestre{" "}
