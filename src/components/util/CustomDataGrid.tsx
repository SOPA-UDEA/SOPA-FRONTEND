...
export const CustomDataGrid = ({
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(
    columns.map((col: ColumnConfig) =>
      col.renderActions ? "actions" : col.field
    )
  );
  ...
  const finalTableColumns = columns
    .filter((col: ColumnConfig) =>
      visibleColumns.includes(col.renderActions ? "actions" : col.field)
    )
      ...
      <Table
        ...
        <TableBody items={data} emptyContent={"No hay datos para mostrar."}>
          {(item) => (
            <TableRow
              key={String(
                (item as { id?: unknown; code?: unknown }).id ||
                  (item as { id?: unknown; code?: unknown }).code ||
                  Math.random()
              )}>
              {(columnKey) => {
                const columnConfig = finalTableColumns.find(
                  (col: {
                    key: string;
                    label: string;
                    renderActions?: (item: unknown) => React.ReactNode;
                ...
                if (columnConfig?.renderActions) {
                  return (
                    <TableCell>{columnConfig.renderActions(item)}</TableCell>
                  );
                }
                return (
